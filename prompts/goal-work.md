...
The brief's intent selects the target automatically. Review/improve without work returns needs_work. No goal or archive intent: omit --brief; needs_goal asks once. “Just save for later”: --action save, no extraction. “Save these as NAME”: --action prepare, which extracts/reconciles broadly useful knowledge and ends at knowledge_saved without a brief, result or skill. Continue prepare with --reconciled only after reviewing extraction. Explore-only uses --action explore and can stop at explored. The older --target checklist remains for legacy runs; new checklist goals use intent plan.
