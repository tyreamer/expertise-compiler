...
Use the saved map's recommended order (up to five). Lead with useful names, then concise give/get, why supported, repeat-use value and important limits. Formats are secondary. Don't show internal assessment tables by default. Recommend the strongest first build with its reason, and invite a choice conversationally. Never announce a fixed number of strong opportunities if support is mixed or absent.
